...
def report(
    ...
    min_missing_share: float = typer.Option(
        0.1,
        help="Порог доли пропусков (от 0.0 до 1.0), выше которого колонка считается проблемной."
    ),
) -> None:
    """
    ...
    with md_path.open("w", encoding="utf-8") as f:
        ...
        if problematic_missing_cols:
            f.write(f"- Проблемные колонки (пропусков ≥ {min_missing_share:.1%}): **{', '.join(problematic_missing_cols)}**\n")
        else:
            f.write(f"- Проблемных колонок по пропускам нет.\n")
        f.write("\n")
        ...
        if not top_cats:
            ...
            f.write(f"Для каждой категориальной колонки показаны top-{top_k_categories} значений.\n")
