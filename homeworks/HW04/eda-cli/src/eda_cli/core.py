...
def compute_quality_flags(summary: DatasetSummary, missing_df: pd.DataFrame) -> Dict[str, Any]:
    ...
    high_card_cols = [
        col for col in summary.columns
        if not col.is_numeric and col.non_null > 0 and (col.unique / col.non_null) > 0.95
    ]
    flags["has_high_cardinality_categoricals"] = len(high_card_cols) > 0

    # 3. Подозрительные дубликаты в ID-колонках
    id_like_cols = [col for col in summary.columns if col.name.lower().endswith('_id') or col.name.lower() == 'id']
    has_id_dup = False
    for col in id_like_cols:
        if col.non_null > 0 and col.unique < col.non_null:
            has_id_dup = True
            break
    flags["has_suspicious_id_duplicates"] = has_id_dup
